...
def build_masks(tile, for_imagery=False, ctx=None):
    ...
    dest_dir = (
        ...
        if not for_imagery
        else os.path.join(FNAMES.mask_dir(tile.lat, tile.lon), "Combined_imagery")
    )
    ...
    (dico_sea, dico_inland) = record_water_tris(
        tile,
    )
    ...
    def build_mask(til_x, til_y):
        ...
        blured_mask = numpy.maximum(
            (pre_mask > 0).astype(numpy.uint8) * 255, blured_mask
        ...
        if not (blured_mask.max() == 0 or blured_mask.min() == 255):
            ...
            mask_im.save(os.path.join(dest_dir, FNAMES.legacy_mask(til_x, til_y)))
            del blured_mask
            ...
            if tile.distance_masks_too:
                ...
                masks_im.save(
                    os.path.join(dest_dir, FNAMES.distance_mask(til_x, til_y))
                )
                UI.vprint(
                    1,
                    "   Created",
                    ...
                    "and",
                    FNAMES.distance_mask(til_x, til_y),
                )
            else:
...
def build_dem_pre_mask(til_x, til_y, tile):
    ...
    (
        ...
        demarr4326,
    ) = tile.dem.super_level_set(mask_altitude_above, (lonmin, lonmax, latmin, latmax))
    if demarr4326.any():
        ...
        dem_array = (numpy.array(demim3857, dtype=numpy.uint8) > 0).astype(
            numpy.uint8
        ) * 255
        del demim3857
        del demim4326
    ...
    for mesh_file_name in mesh_list:
        ...
        for i in range(0, nbr_tri_in):
            ...
            if (
                ...
                or ((tri_type & has_water) < 2 and not tile.use_masks_for_inland)
            ):
                continue
            ...
            if (til_x, til_y) in dico_sea:
                dico_sea[(til_x, til_y)].append((lat1, lon1, lat2, lon2, lat3, lon3))
            else:
                dico_sea[(til_x, til_y)] = [(lat1, lon1, lat2, lon2, lat3, lon3)]
            if a == 0:
...
def blur_mask(img_array, tile, sea_level):
        ...
        if blur_width:
            ...
            b_img_array = (
                (
                    (
                        numpy.tan(
                            (b_img_array.astype(numpy.float32) - 127.5) / 128 * atan(3)
                        )
    ...
    # 3 steps
        ...
        for i in range(stepsin):
            value = shore_level + transition_profile((i + 1) / stepsin, "parabolic") * (
        ...
        b_mask_array = (
            numpy.array(
                Image.fromarray(b_mask_array)
                .convert("L")
                .filter(ImageFilter.GaussianBlur(sea_b_radius_buffered - sea_b_radius)),
                dtype=numpy.uint8,
            )
            == 255
        ).astype(numpy.uint8) * 255
    ...
    if buffer_width:
        ...
        if buffer_width > 0:
            mask_im = Image.fromarray(
                (numpy.array(mask_im, dtype=numpy.uint8) > 0).astype(numpy.uint8) * 255
            )
        else:  # buffer width can be negative
            mask_im = Image.fromarray(
                (numpy.array(mask_im, dtype=numpy.uint8) == 255).astype(numpy.uint8)
                * 255
            )
    if mask_width:
        mask_width += 1
