...
from O4_CLI_Packages import upgrade_package_command, validate_package_command
from O4_CLI_Scenery import dispatch_scenery
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "validate-package":
        return validate_package_command(args.package_dir)

    if args.command == "upgrade-package":
        return upgrade_package_command(
            args.package_dir, dry_run=args.dry_run, update_scenery=args.update_scenery
        )
...
def _parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument(
        "argv", nargs=argparse.REMAINDER, help="Scenery subcommand and args"
    )
