...
def compute_depth_ratio_bounds_from_masks(nbr_nodes, node_coords, node_types, tile):
    ...
    for mask_attr in mask_to_nodes:
        mask_file = os.path.join(
            FNAMES.mask_dir(tile.lat, tile.lon), FNAMES.distance_mask(*mask_attr)
        )
        ...
        tex_x, tex_y = mask_attr
        for n in mask_nodes:
            lon = node_coords[5 * n + 0]
            lat = node_coords[5 * n + 1]
            (s, t) = GEO.st_coord(lat, lon, tex_x, tex_y, tile.mask_zl, None)
            pixx = int(s * 4095)
            pixy = int((1 - t) * 4095)
