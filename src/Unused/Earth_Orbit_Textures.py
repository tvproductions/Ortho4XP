...
if __name__ == "__main__":
    ...
    if ("Earth Orbit Textures" not in tdir) or (not os.path.isdir(tdir)):
        print(
            "Error, target directory does not exist or is not an Earth Orbit Textures directory."
        )
        sys.exit()
    if not os.path.isdir(os.path.join(tdir, "XP_Orig")):
        os.makedirs(os.path.join(tdir, "XP_Orig"))
    for f in os.listdir(tdir):
        ...
        if lon == -180:
            lon = -179.9999
        try:
            (success, im) = IMG.build_texture_from_bbox_and_size(
                (lon, lat + 10, lon + 10, lat),
                "4326",
                (4096, 4096),
                IMG.providers_dict[sys.argv[2]],
            )
        ...
        if success:
            ...
            conv_cmd = [
                ...
                "-bc1",
                "-fast",
                os.path.join(tdir, f.replace(".dds", ".png")),
                os.path.join(tdir, f),
            ]
            tentative = 0
            while True:
                ...
                if tentative == 10:
                    IMG.UI.lvprint(
                        1, "ERROR: Could not convert texture", f, "(10 tries)"
                    )
                    break
                time.sleep(1)
