...
def forest_classifier(pol, lat, lon, tags_dico, dem, forest_dico):
    if not pol.is_valid:
        return
    if (
        VECT.length_in_meters(pol.exterior) < 1000
        and pol.exterior.length**2 > 80 * geometry.Polygon(pol.exterior).area
    ):
...
def OSM_to_MultiPolygon_dico(osm_layer, lat, lon, dem, classifier, dico):
    ...
    for relid in osm_layer.dicosmfirst["r"]:
        ...
        try:
            ...
            multiin = ops.cascaded_union([geom for geom in multiin if geom.is_valid])  # ty:ignore[unresolved-attribute]
            multipol = multiout.difference(multiin)
        except Exception as e:
            UI.lvprint(
                2,
                "Invalid OSM relation starting at",
                osm_layer.dicosmn[osm_layer.dicosmr[relid]["outer"][0]],
                ", skipped.",
            )
            UI.vprint(3, e)
            done += 1
    ...
    if not os.path.isdir(os.path.dirname(dsf_txt_filename)):
        try:
            ...
            UI.exit_message_and_bottom_line(
                ...
                + str(os.path.dirname(dsf_txt_filename))
            )
            return 0
    try:
...
def build_road_exclusion(lat, lon, road_level):
    ...
    if not OSM.OSM_queries_to_OSM_layer(
        queries, road_layer, lat, lon, tags_of_interest, cached_suffix="big_roads"
    ):
        return 0
    road_network = OSM.OSM_to_MultiLineString(road_layer, lat, lon, tags_for_exclusion)
    road_surface = [
        VECT.improved_buffer(linestring, buffer_large, 0, 0, show_progress=False)
        for linestring in road_network
    ]
    if road_level >= 2:
        road_layer = OSM.OSM_layer()
        queries = ['way["highway"="tertiary"]']
        if road_level >= 3:
            queries += ['way["highway"="unclassified"]', 'way["highway"="residential"]']
        if road_level >= 4:
            queries += ['way["highway"="service"]']
        if road_level >= 5:
            queries += ['way["highway"="track"]']
        if not OSM.OSM_queries_to_OSM_layer(
            queries, road_layer, lat, lon, tags_of_interest, cached_suffix="small_roads"
        ):
            return 0
        road_network = OSM.OSM_to_MultiLineString(road_layer, 0, 0, tags_for_exclusion)
        road_surface = geometry.MultiPolygon(
            road_surface
            + [
                VECT.improved_buffer(
                    linestring, buffer_small, 0, 0, show_progress=False
                )
                for linestring in road_network
            ]
        )
    del road_network
    (idx_road, dico_road) = VECT.MultiPolygon_to_Indexed_Polygons(
        road_surface, merge_overlappings=False
    )
...
def build_forest(lat, lon, target_scenery_dir, exclude_road_level=0):
    ...
    if not OSM.OSM_queries_to_OSM_layer(
        queries, forest_layer, lat, lon, tags_of_interest, cached_suffix="forest"
    ):
        return 0
    UI.vprint(1, "    * Building forests multipolygon.")
    OSM_to_MultiPolygon_dico(
        forest_layer, lat, lon, dem, forest_classifier, forest_dico
    )
    ...
    for key in forest_dico:
        ...
        (idx_pol, dico_pol) = VECT.MultiPolygon_to_Indexed_Polygons(
            forest_dico[key], merge_overlappings="True"
        )
        if exclude_road_level:
            print("      Indexed difference with roads")
            (idx_pol, dico_pol) = VECT.indexed_difference(
                idx_pol, dico_pol, idx_road, dico_road
            )
        print("      Cut to tile boundaries")
        forest_dico[key] = geometry.MultiPolygon(
            [
                VECT.cut_to_tile(pol, 0, 1, 0, 1)
                for pol in dico_pol.values()
                if pol.is_valid
            ]
        )
    ...
    dest_dir = os.path.join(
        target_scenery_dir, "Earth nav data", FNAMES.round_latlon(lat, lon)
    )
...
def syntax():
    print(
        "Syntex: python3 src/O4_Forest.py [lat] [lon] [target_scenery_dir] [exclude_road_level]"
    )
