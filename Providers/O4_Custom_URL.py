...
def get_DK_ticket():
    ...
    if (not DK_ticket) or (time.time() - DK_time) >= 3600:
        DK_ticket = "loading"
        tmp = getattr(urllib3_ssl, "DEFAULT_CIPHERS", None)
        if tmp is not None:
            setattr(urllib3_ssl, "DEFAULT_CIPHERS", "HIGH:!DH:!aNULL")
        try:
            DK_ticket = (
                requests.get("https://sdfekort.dk/spatialmap?")
                .content.decode()
                .split("ticket=")[1]
                .split("'")[0]
            )
            ...
            if tmp is not None:
                setattr(urllib3_ssl, "DEFAULT_CIPHERS", tmp)
        DK_time = time.time()
    return DK_ticket
...
def get_DOP40_cookie():
    ...
    if (not DOP40_cookie) or (time.time() - DOP40_time) >= 3600:
        ...
        DOP40_cookie = (
            requests.Session()
            .get(
                "https://sg.geodatenzentrum.de/web_bkg_webmap/lib/bkgwebmap-0.12.4.all.min.js?bkg_appid=4cc455dc-a595-bbcf-0d00-c1d81caab5c3"
            )
            .headers["Set-Cookie"]
            .split(";")[0]
        )
...
def get_Here_value():
    ...
    if (not Here_value) or (time.time() - Here_time) >= 10000:
        ...
        Here_value = (
            str(requests.get("https://wego.here.com" + js_path).content)
            .split('PLATFORM_API_KEY:"')[1][:100]
            .split('"')[0]
        )
...
def custom_tms_request(tilematrix, til_x, til_y, provider):
    if provider["code"] == "NIB":
        ...
        url = (
            "https://maps.hereapi.com/v3/background/mc/"
            + str(tilematrix)
            + "/"
            + str(til_x)
            + "/"
            ...
            + "/jpeg?apikey="
            + Here_value
            + "&style=satellite.day&ppi=100&size=256"
        )
        return (url, fake_headers)
